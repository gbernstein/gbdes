...
public:
  Match(const P &point, set<Cell<P, DIM> *> cellsContainingPoint) : list<const P *>(1, &point),
                                                                    lower(point.getX()),
                                                                    upper(point.getX()),
  ...
  bool touches(const P &point, double rad) const
  {
    ...
    for (int i = 0; i < DIM && possible; i++)
      if (xp[i] < lower[i] - rad || xp[i] > upper[i] + rad)
        possible = false;
    if (!possible)
      return false;
...
public:
  Cell(vector<double> &_lower, vector<double> _upper, Catalog<P, DIM> *_own) : lower(_lower), upper(_upper),
                                                                               owner(_own),
  ...
  void findCellsTouching(const P &point, set<Cell<P, DIM> *> &touching)
  {
    ...
    if (xpt >= splitValue - owner->getRadius())
      right->findCellsTouching(point, touching);
    if (xpt <= splitValue + owner->getRadius())
      left->findCellsTouching(point, touching);
    return;
  }
...
public:
  ...
  Catalog(vector<double> lower, vector<double> upper, double radius_) : radius(radius_), root(lower, upper, this) {}
  ~Catalog()
  {
