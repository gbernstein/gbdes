...
public:
	...
	{
		return extensionNumber < rhs.extensionNumber || (extensionNumber == rhs.extensionNumber && objectNumber < rhs.objectNumber);
	}
...
public:
	FieldsHelper(vector<string> names_,
				 vector<double> ra_,
				 vector<double> dec_,
				 vector<double> epochs_) : names(names_), ra(ra_), dec(dec_), epochs(epochs_){};
