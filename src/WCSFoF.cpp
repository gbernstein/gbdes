...
struct Point
{
  Point(double x_, double y_, long ext, long obj, long expo) : x(2), extensionNumber(ext), objectNumber(obj), exposureNumber(expo)
  {
    x[0] = x_;
    x[1] = y_;
  }
         ...
         char *argv[])
{
  ...
  try
  {
    ...
    for (long iextn = 0; iextn < extensionTable.nrows(); iextn++)
    {
      ...
      if (iextn % 10 == 0)
        cerr << "# Reading object catalog " << iextn
