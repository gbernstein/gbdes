...
ExtensionAttributeBase::ExtensionAttributeBase(string columnName_, Status s) : columnName(columnName_), isInput(false), isOutput(false)
{
  switch (s)
  {
  case ReadOnly:
...
ExtensionAttribute<T>::ExtensionAttribute(string columnName_,
                                          ...
                                          const T &defaultValue_) : ExtensionAttributeBase(columnName_, s),
...
bool ExtensionAttribute<int>::readInputTable(const FTable &inTable, long row)
{
  ...
  else
  {
    ...
    iss >> value;
    if (iss.fail())
      throw std::runtime_error("Could not convert attribute <" + vstring + ">to int");
  }
  return true;
}
...
bool ExtensionAttribute<double>::readInputTable(const FTable &inTable, long row)
{
  ...
  else
  {
    ...
    iss >> value;
    if (iss.fail())
      throw std::runtime_error("Could not convert attribute <" + vstring + ">to double");
  }
  return true;
}
