...
void findDegeneracies(set<int> &degenerateDevices,
                      set<int> &okDevices,
...
void inventoryFitsTables(string inputTables,
  ...
  for (int i = 0; i < names.size(); i++)
  {
    ...
    if (useThisExposure)
    {
      ...
      if (!observatory.empty())
        for (int j = 0; j < 3; j++)
          expo->observatory[j] = observatory[i][j];
...
template <class S>
void fixMapComponents(typename S::Collection &pmc,
...
template <class S>
int findCanonical(Instrument &instr,
                  int iInst,
                  ...
                  typename S::Collection &pmc)
{
  ...
  for (auto const &extnptr : extensions)
  {
    ...
    if (itsExposures.count(iExpo) > 0)
    {
      ...
      if (pmc.dependsOn(extnptr->mapName, exposures[iExpo]->name) && pmc.dependsOn(extnptr->mapName, instr.mapNames[iDev]))
      {
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic, 50) reduction(+ \
                                                         : criticalTime)
#endif
...
template <class S>
void readMatches(vector<int> &seq,
  ...
  for (int i = 0; i <= seq.size(); i++)
  {
    if (i >= seq.size() || seq[i] == 0)
    {
      ...
      if (nValid >= minMatches)
      {
        ...
        {
          if (matchExtns[j] < 0)
            continue; // Skip detections starved of their color
          unique_ptr<typename S::Detection> d(new typename S::Detection);
          d->catalogNumber = matchExtns[j];
          d->objectNumber = matchObjs[j];
          extensions[matchExtns[j]]->keepers.insert(std::pair<long, typename S::Detection *>(matchObjs[j], d.get()));
          if (m)
            m->add(std::move(d));
          else
          {
        ...
        if (matchColorExtension >= 0)
        {
          ...
          colorExtensions[matchColorExtension]->keepers.insert(std::pair<long, typename S::Match *>(matchColorObject,
                                                                                                    matches.back().get()));
        }
      }
...
template <class S>
void readMatches(img::FTable &table,
                 typename S::MCat &matches,
                 const vector<unique_ptr<typename S::Extension>> &extensions,
                 const vector<unique_ptr<typename S::ColorExtension>> &colorExtensions,
                 const ExtensionObjectSet &skipSet,
...
void Photo::fillDetection(Photo::Detection &d,
                          const Exposure *e,
                          astrometry::SphericalCoords &fieldProjection,
                          img::FTable &table, long irow,
                          string xKey, string yKey,
...
#pragma omp parallel for schedule(dynamic, 60) num_threads(CATALOG_THREADS)
  ...
  for (int iext = 0; iext < extensions.size(); iext++)
  {
    ...
    if (!sm)
      cerr << "Exposure " << expo.name << " submap is null" << endl;
...
#pragma omp critical(fitsio)
    ...
    for (long irow = 0; irow < ff.nrows(); irow++)
    {
      ...
      if (pr == extn.keepers.end())
        continue; // Not a desired object
                ...
                bool logging)
{
  ...
  {
    if (!colorExtensions[iext])
      continue; // Skip unused
    auto &extn = *colorExtensions[iext];
    if (extn.keepers.empty())
      continue; // Not using any colors from this catalog
    string filename;
    extensionTable.readCell(filename, "Filename", iext);
    if (logging)
...
template <class S>
void purgeNoisyDetections(double maxError,
                          typename S::MCat &matches,
                          const vector<unique_ptr<Exposure>> &exposures,
                          const vector<unique_ptr<typename S::Extension>> &extensions)
{
...
template <class S>
void purgeSparseMatches(int minMatches,
                        typename S::MCat &matches)
{
...
template <class S>
void purgeBadColor(double minColor, double maxColor,
                   typename S::MCat &matches)
{
  auto im = matches.begin();
  while (im != matches.end())
                            ...
                            const typename S::Collection &pmc)
{
  ...
  for (int iExpo = 0; iExpo < exposures.size(); iExpo++)
  {
    ...
    if (!pmc.mapExists(expoName) || pmc.getFixed(expoName))
      continue; // No free map
    long expoCounts = 0;
    for (int iExtn = 0; iExtn < extensions.size(); iExtn++)
    {
      if (extnCounts[iExtn] == 0)
        continue;
...
template <class S>
void freezeMap(string mapName,
               typename S::MCat &matches,
               vector<unique_ptr<typename S::Extension>> &extensions,
               typename S::Collection &pmc)
{
  // Nothing to do if map is already fixed or doesn't exist
  if (!pmc.mapExists(mapName) || pmc.getFixed(mapName))
    return;
...
template <class S>
void clipReserved(typename S::Align &ca,
                  double clipThresh,
                  ...
                  bool reportToCerr)
{
  ...
  do
  {
    ...
    if (thresh >= max)
      break;
    if (oldthresh > 0. && (1 - thresh / oldthresh) < minimumImprovement)
      break;
    oldthresh = thresh;
...
void Photo::saveResults(const MCat &matches,
                        string outCatalog)
{
...
void Astro::saveResults(const astrometry::MCat &matches,
                        string outCatalog,
                        string starCatalog,
...
void Photo::reportStatistics(const MCat &matches,
                             const vector<unique_ptr<Exposure>> &exposures,
...
void Astro::reportStatistics(const MCat &matches,
                             const vector<unique_ptr<Exposure>> &exposures,
...
#define INSTANTIATE(AP)                                                                                  \
  template vector<unique_ptr<AP::Extension>>                                                             \
