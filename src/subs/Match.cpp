...
void Match::prepare() const
{
  ...
  for (auto const &i : elist)
  {
    ...
    else
    {
      ...
      i->expectedTrueChisq = 2 - (2 * i->fitWeight) * traceATB(i->invCov, invF) + traceATB(i->invCov, trueCentroidCov);
    }
  }
...
int Match::accumulateChisq(double &chisq,
                           DVector &beta,
...
bool Match::sigmaClip(double sigThresh,
                      bool deleteDetection)
{
  solve(); // Recalculate mean position if out of date
  if (dof <= 0)
    return false; // Already degenerate, not fitting
             ...
             bool dump) const
{
  ...
  double chi = 0.;
  if (dof <= 0)
    return chi; // No info from degenerate/perfect fits
  Vector2 dxy;
  for (auto const &i : elist)
  {
    if (!isFit(*i))
...
void PMMatch::prepare() const
{
  ...
  for (auto const &i : elist)
  {
    if (auto ii = dynamic_cast<PMDetection *>(i.get()))
    {
      ii->expectedTrueChisq = 5. - 2 * ii->fitWeight * traceATB(ii->pmInvCov, pmInvFisher) + traceATB(ii->pmInvCov, pmTrueCov);
    }
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          i->expectedTrueChisq = 2. - 2 * i->fitWeight * traceATB(tmp, pmInvFisher) + traceATB(tmp, pmTrueCov);
        }
      }
    }
  }
               ...
               bool dump) const
{
  ...
  {
    if (dump)
      cerr << " Det " << j << " weight " << i->fitWeight << endl;
    j++;
    if (!isFit(*i))
      continue;
    if (auto ii = dynamic_cast<const PMDetection *>(i.get()))
...
bool PMMatch::sigmaClip(double sigThresh,
                        bool deleteDetection)
{
  solve(); // Get PM solution
  if (dof <= 0)
    return false; // Already unusable for fitting
...
int PMMatch::accumulateChisq(double &chisq,
                             DVector &beta,
                             SymmetricUpdater &updater,
                             bool reuseAlpha)
{
  ...
  if (dof <= 0)
    return 0; // No contribution for degenerate fits
  ...
  for (auto i = elist.begin(); i != elist.end(); ++i, ++ipt)
  {
    if (!isFit(**i))
      continue;
    if (dynamic_cast<const PMDetection *>(i->get()))
      continue; // No free params in PMDetections
...
void CoordAlign::operator()(const DVector &p, double &chisq,
...
#pragma omp single
    {
      ...
      for (auto i : mlist)
      {
        ...
        if (j % chunk == 0 && iRound < nRounds)
        {
          ...
          // from previous one.
          if (iRound < nRounds)
            j = (iThread * nRounds + iRound) * chunk;
        }
      }
    }
...
#pragma omp for schedule(dynamic, chunk)
    for (int i = 0; i < vi.size(); i++)
    {
      auto m = vi[i];
      if (m->getReserved())
        continue; //skip reserved objects
...
#pragma omp critical(beta)
  ...
  for (auto const &i : mlist)
  {
    Match *m = i.get();
    if (matchCtr % 10000 == 0)
      cerr << "# accumulating chisq at match # "
           << to_string(matchCtr)
           << endl;
    matchCtr++;
    if (m->getReserved())
...
CoordAlign::fitOnce(bool reportToCerr, bool inPlace)
{
...

  {
    ...
    timer.stop();
    if (reportToCerr)
      cerr << "..fitOnce alpha time " << to_string(timer) << endl;
    timer.reset();
    timer.start();
    ...
    for (int newtonIter = 0; newtonIter < MAX_NEWTON_STEPS; newtonIter++)
    {
      ...
      timer.stop();
      if (reportToCerr)
        cerr << "..solution time " << to_string(timer) << endl;
      timer.reset();
      timer.start();
      DVector newP = p + beta;
      setParams(newP);
      ...
      if (reportToCerr)
      {
        cerr << "....Newton iteration #" << to_string(newtonIter) << " chisq " << to_string(out_chi) << sci
             << " / " << to_string(dof)
...
int CoordAlign::sigmaClip(double sigThresh, bool doReserved, bool clipEntireMatch,
                          bool logging)
{
  int nclip = 0;
...
#pragma omp parallel for schedule(dynamic, chunk) reduction(+ \
                                                            : nclip)
  ...
  timer.stop();
  if (logging)
    cerr << "-->Sigma clipping done in " << to_string(timer) << " sec" << endl;
  return nclip;
}
...
void CoordAlign::count(long int &mcount, long int &dcount,
