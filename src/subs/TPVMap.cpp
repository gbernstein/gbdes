       ...
       string name)
{
  ...
  if ((ctype1 != "RA---TAN" || ctype2 != "DEC--TAN") && (ctype1 != "RA---TPV" || ctype2 != "DEC--TPV") && (ctype1 != "RA---TNX" || ctype2 != "DEC--TNX"))
...
void WriteCD(const LinearMap &lm,
...
ReadPV(const Header *h, int ipv)
{
  ...
  for (h->rewind(); !h->atEnd(); h->incr())
  {
    ...
    if (index == 3 || index == 11 || index == 23 || index == 39)
      continue;
    if (index >= 39)
      index--;
    if (index >= 23)
  ...
  int order = 1;
  while ((order + 1) * (order + 2) / 2 < coeffs.size())
    order++;
...
astrometry::writeTPV(const Wcs &w)
{
  ...
  else
  {
    ...
    if (!sm)
      throw AstrometryError("writeTPV(): input Wcs <" + w.getName() + "> has wrong type of PixelMap <" + sm->getName() + ">");
    if (sm->nMaps() < 1 || sm->nMaps() > 2)
      throw AstrometryError("writeTPV(): input Wcs <" + w.getName() + "> has SubMap <" + sm->getName() + "> with wrong number of PixelMap components");
    lm = dynamic_cast<const LinearMap *>(sm->getMap(0));
    if (!lm)
      throw AstrometryError("writeTPV(): First component of input Wcs <" + w.getName() + "> is not LinearMap");
    if (sm->nMaps() > 1)
    {
      pv = dynamic_cast<const PolyMap *>(sm->getMap(1));
      if (!pv)
        throw AstrometryError("writeTPV(): Second component of input Wcs <" + w.getName() + "> is not PolyMap");
  ...
  if (!gn)
    throw AstrometryError("writeTPV(): input Wcs <" + w.getName() + "> does not have Gnomonic projection.");
...
// to an arbitrary Wcs
//////////////////////////////
Wcs *astrometry::fitTPV(Bounds<double> b,
                        const Wcs &wcsIn,
