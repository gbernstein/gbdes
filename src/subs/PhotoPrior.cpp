...
int PhotoPrior::accumulateChisq(double &chisq,
                                DVector &beta,
                                SymmetricUpdater &updater,
                                bool reuseAlpha)
{
  if (dof < 0)
    return 0; // Can't use this Prior if it's degenerate.
  remap();
  ...
  for (auto &i : points)
  {
    ...
    if (mIsFree)
      derivs2[index++] = -1.;
    if (aIsFree)
      derivs2[index++] = -(i.airmass - 1.);
    if (bIsFree)
      derivs2[index++] = -i.args.color;
    if (cIsFree)
      derivs2[index++] = -i.apcorr;
    ...
    for (int iMap = 0; iMap < i.map->nMaps(); iMap++)
    {
      ...
      if (!reuseAlpha)
      {
        ...
        if (nParams() > 0)
        {
          ...
          updater.rankOneUpdate(mapNumber2, startIndex(), derivs2,
    ...
    if (nParams() > 0)
    {
      ...
      if (!reuseAlpha)
        updater.rankOneUpdate(mapNumber(), ip, derivs2, wt);
    }
