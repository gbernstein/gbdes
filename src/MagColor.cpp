...
int main(int argc, char *argv[])
{
  ...
  try
  {
...

    {
      ...
      if (haveMJD)
        ff.writeCell(0., "MJD", magTableExposureNumber);
      if (haveEpoch)
        ff.writeCell(string(), "Epoch", magTableExposureNumber);
      ...
      for (auto &c : colorList)
      {
        ...
        if (haveMJD)
          ff.writeCell(0., "MJD", c.exposureNumber);
        if (haveEpoch)
          ff.writeCell(string(), "Epoch", c.exposureNumber);
      }
    ...
    for (int i = 0; i < extensionTable.nrows(); i++)
    {
      ...
      if (!exposures[iExposure])
        continue; // Not an extension of interest
    ...
    list<string> extantCols;
    {
      ...
      for (auto colname : vCols)
        extantCols.push_back(colname);
    }
    ...
    ***/
    ...
    for (int icat = 0; icat < catalogHDUs.size(); icat++)
    {
      ...
      {
        if (!extensions[extn[j]])
          continue;
        if (clipSet(extn[j], obj[j]))
...
#pragma omp critical(fitsio)
    ...
    for (int icat = 0; icat < catalogHDUs.size(); icat++)
    {
      ...
      for (int j = 0; j <= extnIn.size(); j++)
      {
        ...
        if (j == extnIn.size())
          break; // Quit if we don't have new data
